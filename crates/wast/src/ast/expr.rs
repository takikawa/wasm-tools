...
enum Try<'a> {
    ...
    Do(Instruction<'a>),
    /// Next thing to parse is `catch`/`catch_all`, `delegate`, or `unwind`.
    CatchDelegateOrUnwind,
...
impl<'a> ExpressionParser<'a> {
        ...
        while !parser.is_empty() || !self.stack.is_empty() {
            ...
            match self.paren(parser)? {
                ...
                Paren::Right => match self.stack.pop().unwrap() {
                    ...
                    Level::Try(Try::CatchDelegateOrUnwind) => {
                        return Err(parser.error("previous `try` had no `catch`, `catch_all`, `delegate`, or `unwind`"));
                    }
                    Level::Try(_) => {
                        self.instrs.push(Instruction::End(None));
                    }
    ...
    fn handle_try_lparen(&mut self, parser: Parser<'a>) -> Result<bool> {
        ...
        if let Try::CatchDelegateOrUnwind = i {
            ...
            // `delegate` has no body, and also ends the `try`.
...
instructions! {
    pub enum Instruction<'a> {
        ...
        CatchAll : [0x05] : "catch_all",
        Unwind : [0x0a] : "unwind",
        Delegate(ast::Index<'a>) : [0xdd] : "delegate", // FIXME: placeholder
